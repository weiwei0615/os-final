...
Thread::Thread(char* threadName, int threadID, int Pri)
{
	ID = threadID;
    name = threadName;
    priority = Pri;
    approximated_CpuBurst = 0;
    CpuBurst = 0;
    age = 0;
    YieldCount = 0;
    stackTop = NULL;
...
//----------------------------------------------------------------------
void
Thread::guessNextBurst()
{
    double oldapproximated = approximated_CpuBurst;
    
    approximated_CpuBurst = 0.5 * CpuBurst + 0.5 * approximated_CpuBurst;
    DEBUG(demo, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << ID << "] update approximated burst time from [" << oldapproximated << "], add [" << CpuBurst << "], to [" << approximated_CpuBurst << "]");
}
void
...
Thread::SelfTest()
{
    ...
    Thread *t = new Thread("forked thread", 1, 149);
...
void
Thread::updateCpuBurst(bool Yield)
{
    CpuBurst += (kernel->stats->totalTicks - startTime/* - YieldCount*/);
...
Thread::aging()
{
    ...
    if(age >= 15) {
        ...
        DEBUG(demo, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" << ID << "] changes its priority from [" << oldpriority << "] to [" << priority);
    }
}
